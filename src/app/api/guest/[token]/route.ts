...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  try {
    ...
    ).all(reservation.property_id || db.prepare('SELECT id FROM properties LIMIT 1').get()?.id);
    ...
    const services = db.prepare(
      ...
      reservation.property_id || db.prepare('SELECT property_id FROM reservations WHERE id = ?').get(reservation.id)?.property_id,

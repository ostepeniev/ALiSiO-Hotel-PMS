...
/**
 * POST /api/crm/leads/from-bot
 * 
 * Called by Telegram bot when user confirms lead creation from inquiry.
 * Creates lead (with dedup) + conversation + message, then triggers AI auto-response
 * which generates a draft and sends it to Telegram for approval.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      adults = 0,
      children = 0,
      type = 'unknown',    // camping | glamping | unknown
      language = 'en',
    } = body;
    ...
    db.prepare(`
      UPDATE crm_conversations SET last_message_at = ?, updated_at = ? WHERE id = ?
    `).run(now, now, conv.id);

    // ── Determine email account ──
    // camping → gmail (kempcarlsbad@gmail.com), glamping → future account
    const accountId = type === 'glamping' ? 'gmail' : 'gmail';

    // ── Generate AI auto-response → sends to Telegram ──

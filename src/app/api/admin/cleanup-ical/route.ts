/**
 * GET  /api/admin/cleanup-ical  — preview what will be deleted
 * DELETE /api/admin/cleanup-ical  — actually delete iCal-synced bookings
 * 
 * Identifies and removes reservations imported via iCal sync:
 *  - id LIKE 'r_ical_%'
 *  - external_uid LIKE 'ical_%'
 *  - source matching known OTA codes with no hostex_reservation_code
 */
...
export async function GET() {
  try {
    ...
    // Find all iCal-synced reservations
    ...
    // Count iCal-only guests (no other reservations)
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Safety check: only delete if query param ?confirm=yes
    ...
    const tx = db.transaction(() => {
      ...
      // 3. Delete iCal-only guests (no remaining reservations)

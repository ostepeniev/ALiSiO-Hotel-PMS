        ...
        {CAMPING_ITEMS.map(item => {
          ...
          return (
            <div key={item.code} onClick={() => toggleItem(item.code)}
              style={{
                position: 'relative', padding: '14px 10px', borderRadius: 'var(--kc-radius-sm)',
                background: active ? 'var(--kc-green-light)' : 'var(--kc-card-bg)',
                border: active ? '2px solid var(--kc-green)' : '2px solid var(--kc-border)',
                cursor: 'pointer', textAlign: 'center', transition: 'all 0.2s ease',
                userSelect: 'none',
              }}>
              {active && (
                <div style={{ position: 'absolute', top: 6, right: 6, width: 20, height: 20, borderRadius: 5,
                  background: 'var(--kc-green)', display: 'flex', alignItems: 'center', justifyContent: 'center',
                  color: '#fff', fontSize: 12, fontWeight: 700 }}>✓</div>
              )}

...
async function sendTg(text: string) {
  if (!BOT_TOKEN || !ADMIN_CHAT_ID) { console.warn('[Teya-Bot] ALISIO_BOT_TOKEN or TELEGRAM_CHAT_ID not set'); return; }
  try {
  ...
  } catch (err) { console.error('[Teya-Bot] Failed to send Telegram message:', err); }
}

...
export async function listExpenses(request: NextRequest): Promise<NextResponse> {
  try {
    const db = getDb();
    ...
    let where = 'WHERE 1=1';
    const params: any[] = [];
    if (month) { where += ' AND e.month = ?'; params.push(month); }
    if (category_id) { where += ' AND e.category_id = ?'; params.push(category_id); }
    if (business_unit_id) { where += ' AND e.business_unit_id = ?'; params.push(business_unit_id); }
    if (search) {
    ...
    const expenses = db.prepare(`
      ...
      ${where} ORDER BY e.expense_date DESC, e.created_at DESC LIMIT ? OFFSET ?
    `).all(...params, limit, offset);

    const categorySummary = db.prepare(`
      SELECT ec.id, ec.name, ec.icon, ec.color, ec.std_group, COALESCE(SUM(e.amount), 0) as total
      FROM expense_categories ec LEFT JOIN expenses e ON e.category_id = ec.id ${month ? 'AND e.month = ?' : ''}
      WHERE ec.is_active = 1 GROUP BY ec.id ORDER BY ec.sort_order
    `).all(...(month ? [month] : []));

    return NextResponse.json({ expenses, total: countRow.total, page, limit, categorySummary });
  } catch (error: any) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}

export async function createExpense(request: Request): Promise<NextResponse> {
  try {
    const db = getDb();
